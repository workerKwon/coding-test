...
m = list(map(int, input().split())) # 각 사람당 인출하는데 걸리는 시간 입력
m.sort() # 인출하는데 걸리는 시간을 오름차순으로 정렬

tmp = 0 # 각 사람들이 인출하기까지 걸리는 시간 초기화
result = 0 # 모두 인출하는데 걸리는 시간 초기화
for time in m:
    tmp += time # 매번 걸리는 시간을 더해준다.
    result += tmp # 각 사람이 인출하기까지 걸리는 시간을 모두 더해준다.
print(result)
