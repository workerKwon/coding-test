...
올해 새롭게 APC의 총감독을 맡게 된 준표는 대회 출제 과정 중 큰 고민에 빠졌다. APC에 참가하는 참가자들이 너무 다양해 대회 문제 난이도 설정이 너무 어렵기 때문이다.
...
# n: 문제 개수, l:풀수 있는 난이도, k: 풀수 있는 문제 개수
n,l,k = map(int, input().split())
result = 0
sub2 = 0 # 풀 수 있는 어려운 문제의 개수 초기화
sub1 = 0 # 풀 수 있는 쉬운 문제의 개수 초기화
for i in range(n): # 문제 개수 만큼 쉬운 문제와 어려운 문제의 난이도 입력
    m = list(map(int, input().split()))
    if m[1] <= l:
        sub2 += 1 # 어려운 문제의 개수 1 증가
    elif m[0] <= l:
        sub1 += 1 # 쉬운 문제의 개수 1 증가

result += min(sub2, k) * 140 # 풀수 있는 최대 문제 수와 어려운 문제의 개수중 작은 것을 골라 140을 곱해서 결과에 더한다.
if sub2 < k: # 만약 풀 수 있는 어려운 문제의 개수가 최대 풀 수 있는 문제의 개수보다 적으면
    result += min(k-sub2, sub1) * 100 #최대 풀 수 있는 문제 개수에서 풀수 있는 어려운 문제의 개수를 빼고, 풀 수 있는 쉬운 문제 개수와 비교해서 작은 것에 100을 곱해서 결과에 더한다.
